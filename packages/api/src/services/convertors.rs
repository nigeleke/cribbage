use backend::{
    AvailableGame, AvailableGameSource, Card, CutForDealState, Game, PLAYER0, PLAYER1, Pegging,
    Player, State,
};
use dto::{AvailableGameDTO, CardDTO, CutForDealStateDTO, GameIdDTO, Score, UserGameDTO};
...
#[inline]
fn player_as_dto(current: Player, required: Player) -> dto::Player {
    if current == required {
        dto::Player::User
    } else {
        dto::Player::Opponent
...
pub fn game_to_user_game_dto(game: &Game, user_id: &backend::UserId) -> UserGameDTO {
    ...
    let view_for_user_opponent = |user: Player, opponent: Player| match game.state() {
        ...
        State::Discarding(state) => {
            ...
            let user_cut = face_up(state.cut(user));
            let opponent_cut = face_up(state.cut(opponent));
            ...
            UserGameDTO::new(game.name())
                .with_dealer_and_crib(user_cut, opponent_cut, dealer, &crib)
