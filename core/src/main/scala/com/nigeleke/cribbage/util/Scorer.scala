...
object Scorer:
  ...
  def forPlay(play: Seq[Play]): PlayPoints =
    ...
    PlayPoints(pairsPoints, fifteensPoints, runsPoints)

  def forEndPlay(plays: Plays): EndPlayPoints =
    ...
    EndPlayPoints(endOfPlayPoints)

  def forCards(cards: Seq[Card], cut: Card): CardsPoints =
    ...
    CardsPoints(pairs = pairs, fifteens = fifteens, runs = runs, flushes = flushes, heels = heels)
