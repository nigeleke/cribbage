...
class CribbageSpec extends AnyWordSpec with Matchers:
  ...
  "Cribbage" should {
    ...
    /** ## The Draw, Shuffle and Cut
      ...
      */
    "draw for dealer" when {
      def validateDraw: Draw => Assertion =
        case Undecided(draws)             => draws.head._2.rank should be(draws.last._2.rank)
      ...
      "redraw" in {
        val draws     = (1 to 2).map(_ => (Player.newPlayer, Card(Jack, Hearts))).toMap
        val priorDraw = Undecided(draws)
        val redraw    = Cribbage.redraw(priorDraw)
        validateDraw(redraw)
      }
