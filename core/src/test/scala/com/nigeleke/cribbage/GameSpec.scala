...
//    val updatedScores = game.scores.updated(player, score)
//    val updatedHands = game.hands.updated(player, hand)
...
//    game.copy(optDealer = Some(player))
//  def withNextToPlay(index: Int): Game =
//    val player = game.players(index)
//    val updatedPlays = game.plays.copy(optNextToPlay = Some(player))
...
//          case Valid(game) =>
//            game.hands(player).toCardSeq should not contain allElementsOf(discards)
