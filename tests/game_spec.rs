...
mod players {
    ...
    fn provide_opponent_to_existing_player() {
        ...
        assert_eq!(game.opponent(opponent).unwrap(), player);
    }
...
mod deck {
    ...
    fn use_a_standard_pack_of_cards() {
        ...
        assert!(!deck.contains(&game.cut(player1).unwrap()));
        assert!(!deck.contains(&game.cut(player2).unwrap()));
    }
}
...
mod deal {
    ...
    fn deal_six_cards_per_player() {
        ...
        players
            .into_iter()
            .for_each(|p| assert_eq!(game.hand(p).unwrap().len(), CARDS_DEALT_PER_HAND));
    }
    ...
    fn deal_when_draw_decided() {
        ...
        players.iter().for_each(|p| {
            assert_eq!(*game.pegging(*p).unwrap().back_peg().points(), 0);
            assert_eq!(*game.pegging(*p).unwrap().front_peg().points(), 0);
        });

        players.iter().for_each(|p| {
            assert_eq!(game.hand(*p).unwrap().len(), CARDS_DEALT_PER_HAND);
        });
...
// TODO: Add tests here for winning states...
...
mod the_crib {
    ...
    fn player_can_discard_one_held_card_to_the_crib() {
        ...
        let player_hand0 = game0.hand(player0).unwrap();
        ...
        let player_hand1 = game1.hand(player0).unwrap();
        let opponent_hand1 = game1.hand(opponent0).unwrap();
    ...
    fn player_cannot_discard_more_then_two_held_cards_to_the_crib() {
        ...
        let error = game0.discard(player0, &discards0).unwrap_err();
    ...
    fn cannot_discard_when_player_not_participating() {
        ...
        let error = game0.discard(non_player, &discards0).err().unwrap();
...
mod the_play {
    ...
    fn accept_valid_play() {
        ...
        let PlayResult::Playing(game1) = game0.play(pone0, Card::from("4S")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn cannot_play_when_not_their_turn() {
        ...
        assert_eq!(error, GameError::CannotPlay);
    }
    ...
    fn cannot_play_when_play_exceeds_target() {
        ...
        let play_state0 = game0.play_state();
        assert_eq!(play_state0.legal_plays(pone0), Hand::from(""));

        let error = game0.play(pone0, Card::from("4S")).unwrap_err();
    ...
    fn score_play_when_target_not_reached_mid_play() {
        ...
        let PlayResult::Playing(game1) = game0.play(pone0, Card::from("5H")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn score_play_when_target_not_reached_end_play() {
        ...
        let PlayResult::Playing(game1) = game0.play(pone0, Card::from("2H")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn score_play_when_target_not_reached_finished() {
        ...
        let PlayResult::Finished(game1) = game0.play(pone0, Card::from("5H")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn score_play_when_target_reached_mid_play() {
        ...
        let PlayResult::Playing(game1) = game0.play(pone0, card).unwrap() else {
    ...
    fn score_play_when_target_reached_finished() {
        ...
        let PlayResult::Finished(game1) = game0.play(pone0, Card::from("AH")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn swap_player_after_dealer_play() {
        ...
        let PlayResult::Playing(game1) = game0.play(dealer0, Card::from("9C")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn reset_play_after_exact_target_reached() {
        ...
        let PlayResult::Playing(game1) = game0.play(dealer0, Card::from("8H")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn score_play_points_for_fifteens() {
        ...
        let PlayResult::Playing(game1) = game0.play(pone0, Card::from("8D")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn score_play_points_for_quartet() {
        ...
        let PlayResult::Playing(game1) = game0.play(pone0, Card::from("7D")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn score_play_points_for_run() {
        ...
        let PlayResult::Playing(game1) = game0.play(pone0, Card::from("AS")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn score_play_points_for_run_edge_case_1() {
        ...
        let PlayResult::Playing(game1) = game0.play(pone0, Card::from("8S")).unwrap() else {
            panic!("unexpected state")
        };
    ...
    fn score_play_points_for_run_edge_case_2() {
        ...
        let PlayResult::Playing(game1) = game0.play(pone0, Card::from("9S")).unwrap() else {
            panic!("unexpected state")
        };
...
mod the_go {
    ...
    fn accept_pass_when_pone_has_no_valid_card() {
        ...
        let PassResult::Playing(game1) = game0.pass(pone0).unwrap() else {
    ...
    fn swap_player_after_dealer_pass() {
        ...
        let PassResult::Playing(game1) = game0.pass(dealer0).unwrap() else {
            panic!("unexpected state")
        };
...
mod counting_the_hands {
    ...
    fn hand_should_score_fifteens() {
        assert_eq!(
            *HandScorer::new(&Hand::from("7H8CAC2C"), Cut::from("4H"))
                .score()
                .points(),
            4
        );
        assert_eq!(
            *HandScorer::new(&Hand::from("THJCKS5H"), Cut::from("4H"))
                .score()
                .points(),
            6
        );
    ...
    fn hand_should_score_pairs() {
        assert_eq!(
            *HandScorer::new(&Hand::from("2H4C5C2C"), Cut::from("AH"))
                .score()
                .points(),
            2
        );
        assert_eq!(
            *HandScorer::new(&Hand::from("TCASADTH"), Cut::from("AH"))
                .score()
                .points(),
            8
        );
    ...
    fn hand_should_score_royal_pairs() {
        assert_eq!(
            *HandScorer::new(&Hand::from("2H2D5C2C"), Cut::from("AH"))
                .score()
                .points(),
            6
        );
    ...
    fn hand_should_score_double_royal_pairs() {
        assert_eq!(
            *HandScorer::new(&Hand::from("2H2C2D2S"), Cut::from("AH"))
                .score()
                .points(),
            12
        );
    ...
    fn hand_should_score_runs() {
        assert_eq!(
            *HandScorer::new(&Hand::from("JDQCKC2C"), Cut::from("AH"))
                .score()
                .points(),
            3
        );
        assert_eq!(
            *HandScorer::new(&Hand::from("3C3S2D5H"), Cut::from("AH"))
                .score()
                .points(),
            8
        );
    ...
    fn hand_should_score_flushes() {
        assert_eq!(
            *HandScorer::new(&Hand::from("2H4H6H8H"), Cut::from("TH"))
                .score()
                .points(),
            5
        );
        assert_eq!(
            *HandScorer::new(&Hand::from("2D4D6D8D"), Cut::from("TH"))
                .score()
                .points(),
            4
        );
    ...
    fn hand_should_score_his_heels() {
        assert_eq!(
            *HandScorer::new(&Hand::from("2D4H6HJH"), Cut::from("TH"))
                .score()
                .points(),
            1
        );
        assert_eq!(
            *HandScorer::new(&Hand::from("2H4D6DJD"), Cut::from("TH"))
                .score()
                .points(),
            0
        );
    ...
    fn crib_should_score_fifteens() {
        assert_eq!(
            *CribScorer::new(&Crib::from("7H8CAC2C"), Cut::from("4H"))
                .score()
                .points(),
            4
        );
        assert_eq!(
            *CribScorer::new(&Crib::from("THJCKS5H"), Cut::from("4H"))
                .score()
                .points(),
            6
        );
    ...
    fn crib_should_score_pairs() {
        assert_eq!(
            *CribScorer::new(&Crib::from("2H4C5C2C"), Cut::from("AH"))
                .score()
                .points(),
            2
        );
        assert_eq!(
            *CribScorer::new(&Crib::from("TCASADTH"), Cut::from("AH"))
                .score()
                .points(),
            8
        );
    ...
    fn crib_should_score_royal_pairs() {
        assert_eq!(
            *CribScorer::new(&Crib::from("2H2D5C2C"), Cut::from("AH"))
                .score()
                .points(),
            6
        );
    ...
    fn crib_should_score_double_royal_pairs() {
        assert_eq!(
            *CribScorer::new(&Crib::from("2H2C2D2S"), Cut::from("AH"))
                .score()
                .points(),
            12
        );
    ...
    fn crib_should_score_runs() {
        assert_eq!(
            *CribScorer::new(&Crib::from("JDQCKC2C"), Cut::from("AH"))
                .score()
                .points(),
            3
        );
        assert_eq!(
            *CribScorer::new(&Crib::from("3C3S2D5H"), Cut::from("AH"))
                .score()
                .points(),
            8
        );
    ...
    fn crib_should_score_flushes() {
        assert_eq!(
            *CribScorer::new(&Crib::from("2H4H6H8H"), Cut::from("TH"))
                .score()
                .points(),
            5
        );
        assert_eq!(
            *CribScorer::new(&Crib::from("2D4D6D8D"), Cut::from("TH"))
                .score()
                .points(),
            0
        );
    ...
    fn crib_should_score_his_heels() {
        assert_eq!(
            *CribScorer::new(&Crib::from("2D4H6HJH"), Cut::from("TH"))
                .score()
                .points(),
            1
        );
        assert_eq!(
            *CribScorer::new(&Crib::from("2H4D6DJD"), Cut::from("TH"))
                .score()
                .points(),
            0
        );
...
mod combinations {
    ...
    fn should_score_rules_example_eights_sevens_sixes() {
        assert_eq!(
            *HandScorer::new(&Hand::from("8H7C7D6S"), Cut::from("2H"))
                .score()
                .points(),
            16
        );
    ...
    fn should_score_rules_example_runs() {
        assert_eq!(
            *HandScorer::new(&Hand::from("JHQCKDAS"), Cut::from("2D"))
                .score()
                .points(),
            3
        );
    ...
    fn should_score_rules_example_flush() {
        assert_eq!(
            *HandScorer::new(&Hand::from("THQHKHAH"), Cut::from("2H"))
                .score()
                .points(),
            5
        );
        assert_eq!(
            *HandScorer::new(&Hand::from("THQHKHAH"), Cut::from("2S"))
                .score()
                .points(),
            4
        );
        assert_eq!(
            *HandScorer::new(&Hand::from("THQHKHAS"), Cut::from("2H"))
                .score()
                .points(),
            0
        );
...
mod a_perfect_29 {
    ...
    fn should_score_rules_example_perfect_29() {
        assert_eq!(
            *HandScorer::new(&Hand::from("5H5C5DJS"), Cut::from("5S"))
                .score()
                .points(),
            29
        );
